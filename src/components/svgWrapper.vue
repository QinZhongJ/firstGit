<template>
  <svg xmlns="http://www.w3.org/2000/svg" :style="svgStyle">
    ...
    <template v-for="(pro, g_index) in $svg.projectList">
      <g class="ctm_gantt-grid-pro">
        <rect
...
export default {
  ...
  computed: {
    svgStyle() {
      ...
      let len = 0;
      for (let i of this.$svg.projectList) {
        len += 1;
        if (i.showDetile) {
          len += i.tasks.length;
        }
  ...
  created() {
    this.setup_options(this.$svg.option);
    this.setup_tasks(this.$svg.task);
    this.change_view_mode();
    this.bind_events();
  },
  methods: {
    getRowY(g_index, t_index) {
      let group = this.$svg.projectList[g_index].tasks;
      let len = g_index;
      for (let i = 0; i < g_index; i++) {
        if (this.$svg.projectList[i].showDetile) {
          len += this.$svg.projectList[i].tasks.length;
        }
      }
